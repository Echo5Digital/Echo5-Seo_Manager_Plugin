...
create_package() {
    ...
    if [[ -f "$zip_name" ]]; then
        rm -f "$zip_name"
        echo -e "${YELLOW}Removed old package${NC}"
    fi
    ...
    if command -v zip &> /dev/null; then
        ...
        powershell.exe -Command "Compress-Archive -Path '$temp_dir/$PLUGIN_SLUG' -DestinationPath '$zip_name' -Force"
    else
        echo -e "${RED}ERROR: Neither zip nor PowerShell found. Cannot create package.${NC}"
        rm -rf "$temp_dir"
        exit 1
    fi
...
git_release() {
    ...
    echo -e "${GREEN}Committed: Version $version${NC}"
...
show_summary() {
    ...
    if [[ "$auto_released" == "true" ]]; then
        ...
        echo "5. Release: gh release create v$new_version --title 'Version $new_version'"
    fi
